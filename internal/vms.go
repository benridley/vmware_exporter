...
func (e *vmwareExporter) retrieveVms(ch chan<- prometheus.Metric) error {
    ...
    for _, vm := range vms {
        ...
        ch <- prometheus.MustNewConstMetric(vmwareVmCpuUsageHzDesc, prometheus.GaugeValue, cpuHertz, vm.Summary.Config.Name, vm.Summary.Config.InstanceUuid)
        memoryUsed := float64(int64(vm.Summary.QuickStats.GuestMemoryUsage) * 1000000000)
        ch <- prometheus.MustNewConstMetric(vmwareVmMemoryUsageBytesDesc, prometheus.GaugeValue, memoryUsed, vm.Summary.Config.Name, vm.Summary.Config.InstanceUuid)
        memoryAvailable := float64(int64(vm.Summary.Config.MemorySizeMB) * 1000000000)
        ch <- prometheus.MustNewConstMetric(vmwareVmMemoryAvailableBytesDesc, prometheus.GaugeValue, memoryAvailable, vm.Summary.Config.Name, vm.Summary.Config.InstanceUuid)
    }
    return nil
}
